...
async fn slack_webhook_blocks_snapshot() {
    ...
    // Redact the dynamic generated_at timestamp so the snapshot is stable
    let stable_json = if let Some(start) = json.find("Generated by ceo-cli at ") {
        let prefix_len = "Generated by ceo-cli at ".len();
        let rest = &json[start + prefix_len..];
        let end = rest.find('"').unwrap_or(rest.len());
        format!("{}{}{}", &json[..start + prefix_len], "[timestamp]", &json[start + prefix_len + end..])
    } else {
        json.clone()
    };
    let value: serde_json::Value = serde_json::from_str(&stable_json).unwrap();
